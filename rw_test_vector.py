...
def test_aes_128():
    ...
    target = [0x39, 0x25, 0x84, 0x1d, 0x2, 0xdc, 0x9, 0xfb, 0xdc, 0x11, 0x85, 0x97, 0x19, 0x6a, 0xb, 0x32]
    obj = AES_ENCRYPTION()
    output = obj.encrypt(key, plaintext)
    assert all([output[i]==target[i] for i in range(len(target))])
...
def test_aes_192():
    ...
    target = [0xBD, 0x33, 0x4F, 0x1D, 0x6E, 0x45, 0xF2, 0x5F, 0xF7, 0x12, 0xA2, 0x14, 0x57, 0x1F, 0xA5, 0xCC]
    obj = AES_ENCRYPTION()
    output = obj.encrypt(key, plaintext)
    assert all([output[i]==target[i] for i in range(len(target))])
...
def test_aes_256():
    ...
    target = [0xf3, 0xee, 0xd1, 0xbd, 0xb5, 0xd2, 0xa0, 0x3c, 0x6, 0x4b, 0x5a, 0x7e, 0x3d, 0xb1, 0x81, 0xf8]
    obj = AES_ENCRYPTION()
    output = obj.encrypt(key, plaintext)
    assert all([output[i]==target[i] for i in range(len(target))])
